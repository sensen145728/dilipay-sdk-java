...
引入sdk依赖（最新发布版本1.0.0），支持：支付、退款、转账、分账等接口。

```xml
  <dependency>
      <groupId>com.dilipay</groupId>
      <artifactId>dilipay-sdk-java</artifactId>
...
完整支付测试代码 `com.dili.dilipay.PayOrderTest`

```java
    // 创建客户端
    DilipayClient dilipayClient = DilipayClient.getInstance(Dilipay.appId, Dilipay.apiKey);

    // 构建请求数据
    String wayCode = "WX_BAR";                           // 支付方式
    PayOrderCreateRequest request = new PayOrderCreateRequest();
    PayOrderCreateReqModel model = new PayOrderCreateReqModel();
    model.setMchNo(Dilipay.mchNo);                       // 商户号
    model.setAppId(dilipayClient.getAppId());            // 应用ID
    ...
    // 发起统一下单
    PayOrderCreateResponse response = dilipayClient.execute(request);

    // 验证返回数据签名
    response.checkSign(Dilipay.apiKey);

    // 判断下单是否返回成功
    response.isSuccess(Dilipay.apiKey)
```

完整退款测试代码 `com.dili.dilipay.RefundOrderTest`
...
```java
    ...
    model.setMchNo(Dilipay.mchNo);                       // 商户号
    model.setAppId(Dilipay.appId);                       // 应用ID
    ...
    // 发起统一退款
    RefundOrderCreateResponse response = dilipayClient.execute(request);
...
完整转账测试代码 `com.dili.dilipay.TransferOrderTest`
...
```java
    ...
    request.setBizModel(model);
    try {
        TransferOrderCreateResponse response = dilipayClient.execute(request);
        _log.info("验签结果：{}", response.checkSign(Dilipay.apiKey));
        // 判断转账发起是否成功（并不代表转账成功）
        if(response.isSuccess(Dilipay.apiKey)) {
